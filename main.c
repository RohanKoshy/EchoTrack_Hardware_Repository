...
#define MAX_CAPTURE_SECONDS		5
#define CAPTURE_SAMPLES		(AUDIO_FS_HZ*MAX_CAPTURE_SECONDS)
...
static NVT_NONCACHEABLE uint32_t g_AudioBuf[CAPTURE_SAMPLES];
static volatile uint32_t g_AudioIndex = 0;
static volatile uint8_t g_CaptureDone = 0;
...
int main(void)
{
    ...
    */
    ...
    uint32_t frameCount = 0;
		...
		int it=0;
		//printf("%u", CAPTURE_SAMPLES);
    g_AudioIndex = 0;   // make sure it's reset before the loop

		while (1)
		{
				// 1) If a new PDMA frame is ready AND we still have space, consume it
				if (g_NewFrame && (g_AudioIndex < CAPTURE_SAMPLES))
				{
						__disable_irq();
						...
						for (int i = 0; i < BUFF_LEN_1; i += 2)
						{
								...
								g_AudioBuf[g_AudioIndex] = s24;
								g_AudioIndex++;

								if (g_AudioIndex >= CAPTURE_SAMPLES)
										break;
						}
				}

				// 2) Stop condition: buffer full
				if (g_AudioIndex >= CAPTURE_SAMPLES)
				{
						printf("capture done, samples: %u\n", g_AudioIndex);
						break;
				}

				// 3) Stop condition: button pressed
				if ((PI->PIN & BIT11) == 0)
				{
						printf("button stop, samples: %u\n", g_AudioIndex);
						break;
				}

				// Otherwise: no frame yet, just loop and wait
		}
			//printf("came here\n");
			//printf("%u", g_AudioIndex);
			for (uint32_t i = 0; i < g_AudioIndex; i++)
			{
					printf("came here\n");
					//printf("%ld\n", (long)g_AudioBuf[i]);
